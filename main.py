...
def test_fn(model, val_loader, save_file_format=None):
    device = model.device
    state = model.training
    # 如需要使用case study，则把model.eval()注释掉
    model.eval()
    scores, labels, edges = [], [], []
    for batch in val_loader:
        batch = move_data_to_device(batch, device)
...
def train(config, model_cls=Recommender):
    ...
    datasets = DRDataset(dataset_name=config.dataset_name, drug_neighbor_num=config.drug_neighbor_num,
                         ...
                         drug_sim_threshold=config.drug_sim_threshold, di_sim_threshold=config.drug_sim_threshold)
    log_dir = os.path.join(f"{config.comment}", f"{config.split_mode}-{config.n_splits}-fold", f"{config.dataset_name}",
                           f"{config.seed}", f"{model_cls.__name__}", time.strftime("%Y-%m-%d", time_stamp),
                           time.strftime("%H-%M", time_stamp))
    config.log_dir = log_dir
    config.n_drugs = datasets.drug_num
    config.n_diseases = datasets.disease_num
    ...
    cv_spliter = CVDataset(datasets, split_mode=config.split_mode, n_splits=config.n_splits,
                           ...
                           is_repeat= False)  # is_repeat用于指定是否需要重复n次K折
    ...
    for split_id, datamodule in enumerate(cv_spliter):
        ...
        score, label, edge, metric = test_fn(model, val_loader, None)
        # 训练集作为最终测试集,以下为case study需要调用的代码，test_fn也需要改代码
        # score, label, edge, metric = test_fn(model, train_loader, None)
    ...
    output_file_name = f"final-{config.dataset_name}-{config.split_mode}-{config.n_splits}-fold"
...
def parse(print_help=False):
    ...
    parser.add_argument("--model", default="SEPN", type=str)
    parser.add_argument("--epochs", default=800, type=int)
    # debug用于控制是使用1折，还是跑完10折，False为跑完完整的10折交叉校验
    ...
    # 下面两条语句也是由于加载参数
    # 加载数据集中最大邻居数以及邻居相似度的阈值
    parser = DRDataset.add_argparse_args(parser)
    # 加载训练过程中使用的一些超参数，如多少折交叉验证等
    parser = CVDataset.add_argparse_args(parser)
    # 加载模型的具体参数，隐因子数为多少
    parser = Recommender.add_model_specific_args(parser)
