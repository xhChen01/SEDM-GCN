...
FullGraphData = namedtuple("FullGraphData", ["dr_sim_filter_mat", "di_sim_filter_mat",
...
class FullGraphDataset(Dataset):
    ...
    def build_data(self):
        ...
        # x = x.to_dense()
        dr_sim_filter_mat = torch.sparse_coo_tensor(indices=u_edge[0], values=u_edge[1], size=(self.size_u, self.size_u))
        di_sim_filter_mat = torch.sparse_coo_tensor(indices=v_edge[0], values=v_edge[1], size=(self.size_v, self.size_v))
        ...
        dr_sim_filter_mat = gcn_norm(edge_index=dr_sim_filter_mat, add_self_loops=False)
        di_sim_filter_mat = gcn_norm(edge_index=di_sim_filter_mat, add_self_loops=False)
        # uv_edge = norm_uv_edge * torch.norm(uv_edge) / torch.norm(norm_uv_edge)
        data = FullGraphData(dr_sim_filter_mat=dr_sim_filter_mat,
                             di_sim_filter_mat=di_sim_filter_mat,
