...
class DRDataset():
    ...
    def __init__(self, dataset_name="Fdataset", drug_neighbor_num=15, disease_neighbor_num=15,
                 drug_sim_threshold=0.5, di_sim_threshold=0.5):
        ...
        """
        drug_edge 包含边索引，格式如下[[1,1,2][2,3,3]],即存在边(1,2)、(1,3)、(2,3)
        包含边的权重（相似度），即(1,2)、(1,3)、(2,3)的相似度
        """
        self.drug_edge = self.build_graph(self.drug_sim, drug_neighbor_num, drug_sim_threshold)
        self.disease_edge = self.build_graph(self.disease_sim, disease_neighbor_num, di_sim_threshold)
    ...
    def build_graph(self, sim, num_neighbor, simThreshold = 0):
        if simThreshold > 0:
            row_index = []
            col_index = []
            for i in range(len(sim)):
                for j in range(len(sim[i])):
                    if i == j or sim[i][j] < simThreshold:
                        continue
                    row_index.append(i)
                    col_index.append(j)
        else:
            ...
            # 类似于快排，numghbor指定位置前的索引都是小于它的，后面的索引都是大于它的
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument("--drug_neighbor_num", default=4, type=int)
        parser.add_argument("--disease_neighbor_num", default=4, type=int)
        parser.add_argument("--drug_sim_threshold", default=0.9, type=float)
        parser.add_argument("--disease_sim_threshold", default=0.9, type=float)
...
class Dataset():
    def __init__(self, dataset, mask, fill_unkown=True, stage="train", **kwargs):
        ...
        if not fill_unkown:
            ...
            # row_idx和col_idx的shape为(drugs_size,dis_size),stack的话，就是每个药品与疾病对
...
class CVDataset(pl.LightningDataModule):
    ...
    def __init__(self, dataset, split_mode="global", n_splits=10,
                 ...
                 dataset_type="FullGraphDataset",is_repeat=False,
                 **kwargs):
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument("--n_splits", default=10, type=int) # 设置为1，则所有数据集都为训练集。
    ...
    def fold_mask_iterator(self, interactions, mode="global", n_splits=10,  row_idx=None, col_idx=None, global_test_all_zero=False, seed=666):
        ...
        if mode=="global":
            ...
            if n_splits==1:
                ...
                if self.is_repeat:
                    kfold = RepeatedKFold(n_splits=n_splits, n_repeats=10)
                else:
                    kfold = KFold(n_splits=n_splits, shuffle=True, random_state=seed)
                # 返回interactions中不为零的索引
                pos_row, pos_col = np.nonzero(interactions)
                # 返回interactions中为零的索引
                ...
                for (train_pos_idx, test_pos_idx), (train_neg_idx, test_neg_idx) in zip(kfold.split(pos_row),
                    ...
                    # np.stack是将一系列的数组合并成一个数组，如下面的数组为train_pos_edge[0]==pos_row[train_pos_idx]
                    # train_pos_edge[1]==pos_col[train_pos_idx]
    ...
    def prepare_data(self):
        ...
        # 先使用KFold将数据集切分成K份包含训练集和测试集的数据，并保存在硬盘里
        import glob
